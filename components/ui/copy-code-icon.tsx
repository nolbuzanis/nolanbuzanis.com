...
const CopyIcon = ({ text }: CopyIconProps): JSX.Element => {
  const [clicked, setClicked] = useState(false);

  const handleButtonClick = () => {
    copyToClipboard(text);
    setClicked((value: boolean) => !value);
  };

  useEffect(() => {
    const id = setTimeout(() => setClicked(false), 2000);
    return () => {
      clearTimeout(id);
    };
  }, [clicked]);
  ...
  return (
    <Button onClick={handleButtonClick} aria-label='Copy code block'>
      {clicked ? 'Copied ' : 'Copy '}
      {clicked ? <CopiedIcon /> : <FileIcon />}
    </Button>
  );
};
